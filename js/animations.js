...
function initCapabilityAnimations() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const capabilityItem = entry.target;
                const progressBar = capabilityItem.querySelector('.capability-progress');
                const percentElement = capabilityItem.querySelector('.capability-percent');
                const percent = parseInt(capabilityItem.getAttribute('data-percent') || 0);
                ...
                observer.unobserve(capabilityItem);
            }
        });
    }, { threshold: 0.5 });
...
function initServiceAreaAnimations() {
    const serviceAreaCards = document.querySelectorAll('.serviceArea-card');
...
function initScrollAnimations() {
    ...
    const cards = document.querySelectorAll('.card, .serviceArea-card, .contact-item');
